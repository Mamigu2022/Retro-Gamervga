...
void genesis_bridge_run_frame(bool draw)
{
    ...
    // Solo configurar buffer VDP si vamos a renderizar
    if (draw) {
    ...
    // Precalcular límite de líneas visibles
    const int visible_lines = (int)screen_height;
    const bool run_audio = g_sound_enabled;

    while (scan_line < lines_per_frame)
    {
        int next_clock = system_clock + VDP_CYCLES_PER_LINE;

        m68k_run(next_clock);
        z80_run(next_clock);

        // Audio por línea solo si ACCURATE=0 y sonido activo
        if (!GWENESIS_AUDIO_ACCURATE && run_audio) {
            gwenesis_SN76489_run(next_clock);
            ym2612_run(next_clock);
        }

        // Renderizar solo líneas visibles y solo si draw=true
        if (draw && scan_line < visible_lines)
            gwenesis_vdp_render_line(scan_line);

        if (scan_line == 0 || scan_line > visible_lines)
            hint_counter = REG10_LINE_COUNTER;

        if (--hint_counter < 0) {
            if ((REG0_LINE_INTERRUPT != 0) && (scan_line <= visible_lines)) {
                hint_pending = 1;
        ...
        if (scan_line == visible_lines + 1) {
            z80_irq_line(0);
        }

        system_clock = next_clock;
    }

    // Audio al final del frame (ACCURATE=1) — solo si sonido activo
    if (GWENESIS_AUDIO_ACCURATE && run_audio) {
